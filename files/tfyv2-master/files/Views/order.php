<?php ob_start();
	...
	$groupListPending	=	$cardDetailsControllerObj->getExistGroupList(0);
	
	if(isset($groupListPending) && count($groupListPending) > 0 && is_array($groupListPending)) {
		$firstGroup 		= 	(array)$groupListPending[0];
		foreach($groupListPending as $key => $value){
			$totalOrder += $value->order_count;
		}
	}
	$groupListPast		=	$cardDetailsControllerObj->getExistGroupList(1);
	
	$bindParams = array();
	
	$condition 			= ' 1 and cd.checkoutStatus = 0 and cd.deletedStatus = 1 and cd.fkUserid = ? ';
	$bindParams[] = $userid;
	
	if(isset($_GET['groupid']) && $_GET['groupid'] != '')
	{
		$condition    .= ' and cg.fkGroupId = ? and cg.fkUserId = ? ';
		$bindParams[]  = $_GET['groupid'];
		$bindParams[]  = $userid;
		$g_id          = $_GET['groupid'];
	}
	else if(isset($_GET['id']) && $_GET['id'] == 'all'){
		$condition  .= " group by cd.id";
	}
...
?>
	...
	<div class="Bodycontent">	
		...
		<div class="inner_container">		
				<div class="breadcrum" ><!-- style="display:<?php //echo $breadcrum_display; ?>;" -->
					<div class="">
						...
						<table id="header_yellow" cellpadding="0" cellspacing="0" border="0" align="center" width="100%" style="display:block;">
							<tr>
								<td id="total_val" class="" style="color: #231F20; font-size: 14px; font-weight: bold; text-align: left; width: 200px; padding-top:10px; padding-left:6px;"><?php if(isset($card_details) && is_array($card_details) && count($card_details)>0){ ?>TOTAL ORDERS $<?php echo $totalorder; ?><?php  } else {  ?>PENDING ORDERS<?php  }  ?></td>
								<td id="checkout" align="right" style="padding-top:8px;padding-right:5px; width:610px;"><!-- 700 -->
									<?php if(isset($card_details) && is_array($card_details) && count($card_details)>0){ ?><a href="checkout<?php if(isset($g_id) && $g_id != ""){ echo "?groupid=".$g_id; } else { echo "?id=all"; } ?>" title="CHECKOUT ALL ORDERS" class="greybut">CHECKOUT ALL ORDERS</a><?php  }  ?>
								</td>
							</tr>						
						</table>
					...
					<div class="subnav">
						<div class="cardmenu" ><!-- style="display:<?php //echo $cardmenu_display; ?>;" -->
							<ul id="ul_cardmenu" style="display:block;">
								<li><a href="order?id=all" class="<?php  if(isset($_GET['id']) && $_GET['id'] == 'all'){ ?>sel<?php  }  ?>">ALL</a></li>
								<li class="select"><a href="order" title="PENDING ORDERS" class="<?php if(isset($_GET['page']) && $_GET['page'] == 'order' && $_GET['id'] != 'all') { ?>sel<?php } ?>">PENDING ORDERS</a></li>
									<ul>
									<?php 
									if(isset($groupListPending) && count($groupListPending) > 0 && is_array($groupListPending)) {
									foreach($groupListPending as $key => $value)
								...
								<li class="select"><a href="past-order" title="PAST ORDERS" class="<?php if(isset($_GET['page']) && $_GET['page'] == 'past-order') { ?>sel<?php } ?>">PAST ORDERS</a></li>
									<ul>
										<?php 
									...
									<tr><td>
										<table cellpadding="0" cellspacing="0" align="center" border="0" width="100%">	
											<tr>
												...
												<th width="40%" align="left" class="Otitle"><?php if(isset($pending_gname_array[$g_id])) echo $pending_gname_array[$g_id]; ?></th>
												<th width="20%" align="right" class="close" ><span style="cursor:pointer;" onclick="closeOrder('<?php echo  $value->id; ?>','<?php echo $rows; ?>','<?php echo $value->totalPrice; ?>');">x</span></th>
											</tr>
											...
											<tr>
												...
												<td class="buttons">
													<a href="javascript:void(0);" title="VIEW ORDER" class="greybut" onclick="largepreview('viewOrder?order_id=<?php echo $value->id; ?>');">VIEW ORDER</a>
													<a href="createCard?editId=<?php echo  $value->id; ?>" title="EDIT ORDER" class="greybut">EDIT ORDER</a>
													<a href="<?php if($value->totalPrice == 0) {?>designCard?id=<?php echo  $value->id;  } else { ?>checkout?id=<?php echo  $value->id; } ?>" title="CHECKOUT" class="yellowbut">CHECKOUT</a>
												</td>
											</tr>
											<tr><td colspan="3" height="10"></td></tr>								
										</table>
				...
				<?php 	    }  ?>
						<div id="err_order" class="error_msg" style="display:none; text-align:center;line-height: 93px;">No Pending Order Found</div>
				<?php } else{ ?>
						<div class="error_msg" style="text-align:center;line-height: 93px;">No Pending Order Found</div>
				<?php }?>				
				   </div>
			</div>
		</div>	
